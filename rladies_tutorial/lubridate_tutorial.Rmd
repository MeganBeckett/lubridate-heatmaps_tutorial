...
activities <- subset(activities, select = c(Activity.Type, Date, Distance, Calories, Time, Avg.HR, 
                                            Max.HR, Avg.Run.Cadence, Max.Run.Cadence, Avg.Pace, Best.Pace,
                                            Elev.Gain, Elev.Loss))

# View the classes of the subset of data
lapply(activities, class)
```

read.csv imported some of the the numbers, dates and times as characters, so let's fix this.
...
```{r}
view(dfSummary(activities), method = 'render')
```
